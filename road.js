...
var ROAD = (function () {
  ...
  /* v23 · seams. His words are joined to ours without a stray comma before the period ("something,.") or a doubled word ("camped at at"). */
  function tidy(s) { return String(s || "").trim().replace(/[\s.,;:!?]+$/, ""); }
  function stageParts(ch) {
    var a = tidy(ch.setOut).replace(/^from\s+/i, ""), b = tidy(ch.camped).replace(/^at\s+/i, ""), out = [];
    if (a) out.push("I set out from " + a + ".");
    if (b) out.push("I camped at " + b + ".");
    return out;
  }
  function stageLine(ch) { return stageParts(ch).join(" "); }
  function lcFirst(s) { s = String(s || ""); return s.charAt(0).toLowerCase() + s.slice(1); }
  /* a line of his that trails off on a comma, semicolon or colon closes with a period on the page; what he typed is kept as typed */
  function endTidy(s) { return String(s || "").trim().replace(/[,;:]$/, "."); }
  ...
  function storyParts(d, stage, crafted) {
    var out = [], c = crafted || {}, byMv = c.byMv || {}, seenMv = {};
    if ((c.start || "").trim()) out.push({ mv: 0, movement: null, label: "", ours: "", text: c.start.trim() });
    chapters(d, stage).forEach(function (ch, i) {
      /* v28 · a movement he has put together is his own prose, one piece; his chapters in that movement are not repeated */
      if (byMv[ch.mv]) { if (!seenMv[ch.mv]) { seenMv[ch.mv] = 1; out.push({ mv: ch.mv, movement: d.movements[ch.mv], label: "", ours: "", text: byMv[ch.mv] }); } return; }
    ...
    if (c.handed) out.push({ mv: -1, movement: null, label: HANDED, ours: "", text: c.handed });
    else { var hd = handed(d, stage); if (hd.length) out.push({ mv: -1, movement: null, label: HANDED, ours: "", text: hd.map(function (l) { return l.text; }).join("\n\n") }); }
    if (finished(d, stage) && d.tail.trim() && !c.tailIn) out.push({ mv: -1, movement: null, label: "", ours: "", text: "Today I am camped at " + d.tail.trim() });
    if ((c.god || "").trim()) out.push({ mv: -1, movement: null, label: "", ours: "", text: c.god.trim() });
    return out;
  }
  function pdf(JsPDF, d, stage, crafted) {
    ...
    var chs = chapters(d, stage), parts = storyParts(d, stage, crafted), lastMv = -1;
    ...
    if (finished(d, stage) && plateData) {
      try {
        doc.addPage(); var ph = H - 2 * M - 30, pw = ph * PLATE_W / PLATE_H; if (pw > W - 2 * M) { pw = W - 2 * M; ph = pw * PLATE_H / PLATE_W; }
        doc.addImage(plateData, "JPEG", (W - pw) / 2, M, pw, ph);
        y = M + ph + 12; para(PLATE_LINE, { font: "helvetica", size: 8.5, color: SOFT, center: true, width: 420, after: 0 });
        foot();
      } catch (e) {}
    }
    if (finished(d, stage)) {
  ...
  /* v25 · Lower Still: the plate a man is left with after the last map. It lives beside road.js in the same folder (road-lower-still.jpg);
     AP_ROAD.plate = a string overrides the address, false hides it. The picture says which way the road runs from the orchard: back down. */
  ...
  function plateURL() {
    ...
    var src = (typeof AP_ROAD_SELF === "string" && AP_ROAD_SELF) || ""; return src ? src.replace(/[^\/]*$/, PLATE_FILE) : "";
  }
  function plateHTML() {
    var u = plateURL(); if (!u) return "";
    return '<figure class="plate"><img src="' + u.replace(/"/g, "&quot;") + '" alt="' + PLATE_ALT.replace(/"/g, "&quot;") + '" loading="lazy" width="' + PLATE_W + '" height="' + PLATE_H + '"><figcaption><span class="lbl">Lower still</span>' + PLATE_LINE + '</figcaption></figure>';
  }
  /* the plate as a data address, for the PDF; fetched once, kept for the session */
  var plateData = null, plateReq = null;
  function loadPlate(cb) {
    var u = plateURL(); if (!u || typeof fetch !== "function") { if (cb) cb(null); return; }
    if (plateData) { if (cb) cb(plateData); return; }
    if (!plateReq) plateReq = fetch(u).then(function (r) { return r.ok ? r.blob() : null; }).then(function (b) {
      return new Promise(function (res) { if (!b) return res(null); var fr = new FileReader(); fr.onload = function () { res(fr.result); }; fr.onerror = function () { res(null); }; fr.readAsDataURL(b); });
    }).then(function (d) { plateData = d || null; return plateData; }).catch(function () { return null; });
    plateReq.then(function (d) { if (cb) cb(d); });
  }
  /* v25 · the map with a trail on it, for the story page: one mark for each chapter, the newest in gold */
  function trailSVG(pts, label) {
    ...
    pts = (pts || []).filter(Boolean); var last = pts[pts.length - 1], d = pts.length > 1 ? pathD(pts) : "";
    return '<svg viewBox="0 0 600 500" role="img" aria-label="' + (label || "The map, with the road he walked drawn over it, one mark for each chapter.") + '">' +
      ...
      (d ? '<path d="' + d + '" fill="none" stroke="#C9A227" stroke-opacity=".55" stroke-width="16" stroke-linecap="round" stroke-linejoin="round"/><path d="' + d + '" fill="none" stroke="#1F2A44" stroke-width="2.5" stroke-linecap="round"/>' : "") +
      pts.slice(0, -1).map(function (p) { return '<circle cx="' + p[0] + '" cy="' + p[1] + '" r="5" fill="#1F2A44"/>'; }).join("") +
      (last ? '<circle cx="' + last[0] + '" cy="' + last[1] + '" r="52" fill="#C9A227" fill-opacity=".22"/><circle cx="' + last[0] + '" cy="' + last[1] + '" r="9" fill="#C9A227" stroke="#1F2A44" stroke-width="2.5"/><text x="' + (last[0] + 16) + '" y="' + (last[1] + 6) + '" ' + F + ' font-size="15.5" font-weight="700" fill="#1F2A44">today</text>' : "") + '</svg>';
  }
  function whereTxt(m) { return m && m.where ? "On the road, your marks sit " + m.where + "." : ""; }
  ...
  function chapterProse(d, i, ch, opts) {
    ...
    var open = [];
    if (B) {
      if (byK.did && byK.did.l.ctx && byK.did.l.ctx.quote) open.push({ kind: "bridge", text: B.again.replace("{step}", tidy(byK.did.l.ctx.quote) + ".") });
      else open.push({ kind: "bridge", text: B.open });
    }
    if (byK.did) open.push({ kind: "his", text: endTidy(byK.did.l.text), li: byK.did.li });
    ...
    qs.sort(function (a, b) { return a.j - b.j; }).forEach(function (q) {
      ...
      var br = bridgeFor(i, q.j, q.side); if (br && !(open.length && open[0].kind === "bridge" && open[0].text === br)) body.push({ kind: "bridge", text: br });
      body.push({ kind: "his", text: endTidy(q.l.text), li: q.li });
    });
    ...
    /* in prose the feeling word follows a colon, so it reads in lower case: "What this stirred in me: hopeful." */
    if (byK.feel) paras.push([{ kind: "bridge", text: (B && B.feel) || "What this stirred in me:" }, { kind: "his", text: lcFirst(byK.feel.l.text), li: byK.feel.li }]);
    return paras;
  }
  ...
  return { chapterProse: chapterProse, proseText: proseText, bridgeFor: bridgeFor, lineKind: lineKind, evidence: evidence, markPart: markPart, markFD: markFD, avg: avg, tidy: tidy, endTidy: endTidy, stageParts: stageParts, worksheetURL: worksheetURL, paperLine: paperLine, moves: moves, fullPart: fullPart, bendLabel: bendLabel, bendGloss: bendGloss, shown: shown, recallLine: recallLine, mark: mark, roadPt: roadPt, chapters: chapters, onLines: onLines, stageLine: stageLine, thread: thread, listParts: listParts,
           segments: segments, pathD: pathD, thenNow: thenNow, text: text, pdf: pdf, storyParts: storyParts, handed: handed, HANDED: HANDED, finished: finished,
...
var ROAD_PARTS = (function () {
  ...
  var PARTS = [
    ...
    { part: "Part Two", name: "Stand at the Crossroads", job: 0, week: 3, ours: "The map only works if you're honest about where you're standing on it.",
      /* v23 · Where I stand: the discipleship side of the map, asked once here so the map has both sides from Part Two on.
         Being in a group is the door (yes or not yet; never scored). What moves a man along that side is relationship, in four depths. Numbers only; the lines beneath come in the parts that teach each depth. DRAFT wording, John's to rule (Sept 25, 2026). */
      stand: {
        head: "Where I stand",
        lead: "One tap, then four short lines, 1 to 5. They place your starting mark on the map. Nobody grades them, and you will not see the numbers again.",
        door: { say: "I meet with the same men at a set time, week by week or month by month.", yes: "Yes", no: "Not yet",
                noNote: "Then this part of the map waits. Your mark goes on it once you are meeting with men.",
                ask: "What would make it easy to say yes to a group of men? One line is enough.", label: "What would make it easy to say yes to a group of men", ex: "A group that meets before work, not on a weeknight. Or: one man asking me to come." },
        Q: [
          { say: "There is a man who could tell you what is going on in my life this week." },
          { say: "There is a man whose life I know well enough to say what is hard for him right now." },
          { say: "I have kept meeting with the same men for more than a year." },
          { say: "When something goes wrong in my life, I tell a man before the week is out." }
        ] },
      Q: [
        q("I know honestly where I stand on the map: a full inner life with too little community, a lot of activity with a thin inner life, or both growing together.", "F", false, { ask: "Which corner, and what does it look like in your week?", label: "The corner I lean toward", ex: "A full inner life with too little community. I read for an hour and talk to no one." }, "know", { lo: { ask: "Think of last week. How much of it was alone with God, and how much with other people?", label: "My last week", ex: "An hour a day alone with Him. One conversation with a man, on Sunday." } }),
  ...
  var V17_P2 = {
   "1.0.hi": { "ask": "Which way do you lean? Tap one, then tap who knows you, and say what that looks like in your week.", "ops": ["A full inner life, too little community", "A lot of activity, a thin inner life", "Both growing together", "Neither, right now"], "opn": ["I lean toward a full inner life with too little community. ", "I lean toward a lot of activity with a thin inner life. ", "I lean toward neither corner; both are growing together. ", "Right now it is neither; both are thin. "], "ask2": "Who knows you:", "ops2": ["My family knows me, no man does", "The men I meet with know me, my family doesn’t", "Both know me", "No one does, right now"], "opn2": ["My family knows me, no man does. ", "The men I meet with know me, my family doesn’t. ", "Both know me. ", "No one does, right now. "], "label": "The way I lean", "ex": "I lean toward a full inner life with too little community. My family knows me, my group doesn’t. I read for an hour and talk to no one." },
   ...
   "1.2.hi": { "ask": "What did you walk away from, and have you come back to it yet? A title is enough.", "label": "What I walked away from", "ex": "The men\u2019s group I quit in 2019. Not yet." },
  ...
  var V17_P3 = {
   ...
   "2.1.more": "Pruning is not a verdict on you. Jesus says the branches that get cut back are the ones already bearing fruit (John 15:2). Scripture is honest that God works through hard things: the trial that tests faith and builds steadiness (James 1:2\u20134), the affliction that turns a man back to the Word (Psalm 119:67, 71), the weakness where His grace proves enough (2 Corinthians 12:9), the discipline a father gives a son he loves (Hebrews 12:6\u201311). None of that makes the sickness good, and none of it means you caused it. It means the Vinedresser has not left the vine. Ask what He is making room for. You do not have to know yet.",
  ...
  Object.keys(FIX).forEach(function (k) { var a = k.split("."), q = PARTS[+a[0]].Q[+a[1]], o = {}, src = q[a[2]], f = FIX[k], x; if (a[2] === "say" || a[2] === "note" || a[2] === "more") { q[a[2]] = f; return; } for (x in src) o[x] = src[x]; for (x in f) o[x] = f[x]; q[a[2]] = o; });
...
var WRITER = (function () {
  ...
  /* v24 · The Word for It: one house list, heavy to light (John, Sept 25, 2026: the nine were too few) */
  var FEEL = ["ashamed", "exposed", "afraid", "angry", "sad", "alone", "numb", "tired", "stuck", "restless", "convicted", "sorry", "tender", "relieved", "seen", "hopeful", "grateful", "steady", "free", "glad"];
  ...
  var P = 2, ALL = {};
  function blank() { return { follow: "", step: 1, all: false, view: "steps", n: [null, null, null, null, null], mid: [null, null, null, null, null], t: ["", "", "", "", ""], o: [null, null, null, null, null], o2: [null, null, null, null, null], root: "", feel: "", feelOwn: "", setOut: "", camped: "", aloud: -1, disc: "", discText: "", between: "", witness: "", door: null, rel: [null, null, null, null], doorText: "" }; }
  function W() { var w = ALL[P] || (ALL[P] = blank()); if (!w.o) w.o = [null, null, null, null, null]; if (!w.o2) w.o2 = [null, null, null, null, null]; if (!w.rel) w.rel = [null, null, null, null]; if (w.door === undefined) w.door = null; if (w.doorText == null) w.doorText = ""; return w; }
  ...
  /* v23 · where his answers put him on the map. No mark until both sides of the map have an answer (John, Sept 25, 2026: through Part Two the old
     arithmetic had measured formation only, so every honest man landed in Aging Fruit). The discipleship side comes from a part's own discipleship
     statements, or, where a part has none, from "Where I stand" in Part Two: the door (in a group, yes or not yet; never scored) and four relationship
     statements. Which way he leans is his own X, drawn beside our mark and never used to place it. */
  function standD(s) { if (!s || s.door !== 1) return null; return ROAD.avg(s.rel || []); }
  function baseline() { var ch = APP.D.chapters[1]; return ch && ch.stand ? ch.stand : null; }
  /* v24 · the mark is CUMULATIVE (John, Sept 25, 2026: one statement per side per part let one honest answer fling the mark across the map).
     Every answer so far on each side goes into the average, so the mark starts wide and settles. A low answer whose line beneath asks for the good work
     (the lo side is marked grew) is good news, not low formation, and is left out. Belief statements stay out. */
  function sideVals(qs, ns, mids) {
    var f = [], dd = [];
    (qs || []).forEach(function (q, i) {
      var v = ns && ns[i]; if (v == null || !q.side) return;
      var low = v <= 2 || (v === 3 && mids && mids[i] === "lo");
      if (low && q.lo && q.lo.grew) return;
      v = q.back ? 6 - v : v; (q.side === "F" ? f : dd).push(v);
    });
    return { f: f, d: dd };
  }
  function cumMark(upto, live) {
    var D = APP.D, f = [], dd = [], any = false;
    for (var i = 0; i <= upto; i++) {
      var pt = ROAD_PARTS[i]; if (!pt) continue;
      var ch = D.chapters[i], ns = null, mids = null, st = null;
      if (i === upto && live) { ns = live.n; mids = live.mid; st = pt.stand ? { door: live.door, rel: live.rel } : null; }
      else if (ch && ch.nums) { ns = ch.nums; st = ch.stand || null; }
      if (!ns) continue;
      var sv = sideVals(pt.Q, ns, mids); f = f.concat(sv.f); dd = dd.concat(sv.d);
      if (st && st.door === 1) (st.rel || []).forEach(function (v) { if (v != null) dd.push(v); });
    }
    return ROAD.markFD(ROAD.avg(f), ROAD.avg(dd));
  }
  function markFor() { return cumMark(P, W()); }
  /* the trail, recomputed from what he answered so a mark saved by an older file is corrected too */
  function trail(n) { var D = APP.D, out = []; for (var i = 0; i < n; i++) { var ch = D.chapters[i]; if (!ch || !ch.nums) continue; var m = cumMark(i, null); if (m) out.push([m.x, m.y]); } return out; }
  function leanPick() {
    var w = W(), q = Q()[0], ops = q && q.hi && q.hi.ops;
    if (!ops || ops.length < 2 || !/lean|stand/i.test(q.say || "") || band(0) !== "hi") return null;
    var k = pickOf(0); if (k == null) return null;
    return k;
  }
  /* his own X: the corner he said he leans toward in Part Two, in his words; the third answer sits on the road */
  function leanOps() { var q = ROAD_PARTS[1] && ROAD_PARTS[1].Q[0]; return (q && q.hi && q.hi.ops) || []; }
  function leanOf(ch) {
    if (!ch) return null;
    if (ch.lean != null) return ch.lean;
    /* a chapter saved by an older file: the tap counts only when he answered the statement high, so the tap was a lean and not a "last week" choice */
    if (ch.nums && ch.nums[0] >= 4 && ch.picks && ch.picks[0] != null && ch.picks[0] <= 3) return ch.picks[0];
    return null;
  }
  function ownX() {
    var k = null;
    if (P === 1) k = leanPick();
    else if (P > 1) k = leanOf(APP.D.chapters[1]);
    if (k == null || k < 0 || k > 3) return null;
    var t = leanOps()[k]; if (!t) return null;
    var pt = k === 0 ? ROAD.markFD(4, 2) : k === 1 ? ROAD.markFD(2, 4) : k === 2 ? ROAD.markFD(3.5, 3.5) : ROAD.markFD(1.5, 1.5);
    return { x: pt.x, y: pt.y, text: t.charAt(0).toLowerCase() + t.slice(1) };
  }
  /* v25 · on the last chapter the map is followed by the plate: the road does not end at the orchard, it runs back down */
  function lastPlate() { if (P !== ROAD_PARTS.length - 1) return ""; ROAD.loadPlate(); return ROAD.plateHTML(); }
  function mapNote() {
    var w = W(), x = ownX(), h = "";
    if (P === 0) h = "The map begins in your next chapter, when you say where you stand.";
    else if (part().stand && w.door === 0) h = "You said you are not in a group of men yet, so the discipleship side of the map waits. Your mark appears once you are in a room with other men.";
    else if (part().stand) h = "Your mark appears once you answer the four short lines under “Where I stand.”";
    else h = "Your mark appears once there is an answer on both sides of the map.";
    if (x) h += " Your own X, in your words: “" + x.text + ".”";
    return h;
  }
  function part() { return ROAD_PARTS[P]; }
  function Q() { return part().Q; }
  function steps() { var s = ["intro"]; if (lastWeek()) s.push("follow"); if (part().evidence) s.push("witness"); Q().forEach(function (q, i) { s.push("q" + i); if (i === 0 && part().stand) s.push("stand"); }); s.push("feel", "stage"); return s; }
  ...
  function qHTML(i) {
    var w = W(), q = Q()[i], b = band(i), side = b ? q[b] : null, h = '<section class="wq"><p class="q">' + esc(q.say) + '</p>' + (q.note ? '<p class="help">' + esc(q.note) + '</p>' : "") + (q.more ? '<details class="more"><summary>Read more</summary><p class="help">' + esc(q.more) + '</p></details>' : "") + '<div class="scale" role="group" aria-label="One to five">';
    ...
    if (w.n[i] === 3) h += '<div class="beneath"><p class="help">Some of each. Answer whichever is truer for you.</p><div class="words">' +
      '<button class="word' + (b === "hi" ? " pick" : "") + '" data-mid="' + i + '-hi" aria-pressed="' + (b === "hi") + '">' + esc(q.hi.pick || q.hi.label) + '</button>' +
      '<button class="word' + (b === "lo" ? " pick" : "") + '" data-mid="' + i + '-lo" aria-pressed="' + (b === "lo") + '">' + esc(q.lo.pick || q.lo.label) + '</button></div></div>';
    if (side) {
  ...
  /* v24 · the Numbers 33 chain: "they set out from Marah and camped at Elim; they set out from Elim…" Last chapter's camp is offered as this chapter's start with one tap. */
  function lastCamp() { var ch = prevCh(); return ch ? ROAD.tidy(ch.camped).replace(/^at\s+/i, "") : ""; }
  function stageHTML() {
    var s = part().stage || STAGE, w = W(), lc = lastCamp();
    return '<section class="wq"><p class="ours left">' + esc(s.text) + ' <span class="ref">' + esc(s.ref) + '</span></p><p class="help">' + esc(s.help) + '</p>' +
      (lc ? '<p class="help">Last chapter you camped at: \u201c' + esc(lc) + '.\u201d Israel set out each stage from where they had camped. You can too.</p><div class="words"><button class="word' + (w.setOut === lc ? " pick" : "") + '" data-camp="1" aria-pressed="' + (w.setOut === lc) + '">Set out from where I camped</button></div>' : "") +
      ...
      '<div class="field"><label for="t-camp">I camped at\u2026</label><input id="t-camp" data-w="camped" value="' + esc(w.camped) + '" placeholder="' + (P === ROAD_PARTS.length - 1 ? "the trailhead, with a man who is just starting" : "one man knows, and that is enough for this week") + '"></div></section>';
  }
  ...
  /* v23 · Where I stand (Part Two): the door, then four relationship statements, numbers only. They place his starting mark; the lines beneath come in the parts that teach each depth. */
  function standHTML() {
    var w = W(), sd = part().stand, h = '<section class="wq"><p class="q">' + esc(sd.head) + '</p><p class="help">' + esc(sd.lead) + '</p>';
    h += '<div class="stand"><p class="q">' + esc(sd.door.say) + '</p><div class="words">' +
      '<button class="word' + (w.door === 1 ? " pick" : "") + '" data-door="1" aria-pressed="' + (w.door === 1) + '">' + esc(sd.door.yes) + '</button>' +
      '<button class="word' + (w.door === 0 ? " pick" : "") + '" data-door="0" aria-pressed="' + (w.door === 0) + '">' + esc(sd.door.no) + '</button></div>';
    if (w.door === 0) h += '<p class="help">' + esc(sd.door.noNote) + '</p><div class="beneath"><p class="ask">' + esc(sd.door.ask) + '</p><label class="sr" for="t-door">' + esc(sd.door.ask) + '</label>' +
      '<textarea id="t-door" data-w="doorText" rows="2" placeholder="One sentence is enough.">' + esc(w.doorText || "") + '</textarea><p class="ex">Something a man could picture. Like: <i>' + esc(sd.door.ex) + '</i></p></div>';
    h += '</div>';
    if (w.door === 1) sd.Q.forEach(function (q, i) {
      h += '<div class="stand"><p class="q">' + esc(q.say) + '</p><div class="scale" role="group" aria-label="One to five">';
      for (var v = 1; v <= 5; v++) h += '<button class="dot' + (w.rel[i] === v ? " on" : "") + '" data-rel="' + i + "-" + v + '" aria-pressed="' + (w.rel[i] === v) + '">' + v + '</button>';
      h += '</div><div class="ends"><span>Strongly disagree</span><span>Strongly agree</span></div></div>';
    });
    return h + '</section>';
  }

  function stepHTML(kind) { return kind === "witness" ? witnessHTML() : kind === "follow" ? followHTML() : kind === "between" ? betweenHTML() : kind === "feel" ? feelHTML() : kind === "stage" ? stageHTML() : kind === "stand" ? standHTML() : qHTML(+kind.slice(1)); }
  ...
  function lines() {
    ...
    Q().forEach(function (q, i) {
      ...
      /* v23 · a man not yet in a group says what would get him into a room; that sentence is his honest first step and goes in his chapter */
      if (i === 0 && part().stand && w.door === 0 && (w.doorText || "").trim()) out.push({ k: "other", label: part().stand.door.label, text: w.doorText.trim() });
    });
  ...
  function trailSoFar() { var D = APP.D; return APP.stage === "me" ? trail(P) : D.trail.slice(0, P); }
  function markSVG(m, x) {
    var F = 'font-family="Source Sans 3, Arial, sans-serif"', FS = 'font-family="Source Serif 4, Georgia, serif" font-size="19" font-style="italic" fill="#8C6A3F"';
    var prev = trailSoFar(), pts = prev.concat([[m.x, m.y]]), d = pts.length > 1 ? ROAD.pathD(pts) : "";
    return '<svg viewBox="0 0 600 500" role="img" aria-label="The map. A wide gold mark shows which way his answers lean in this part' + (x ? ", and an X where he said he stands himself" : "") + '.">' +
      '<rect width="600" height="500" fill="#FFFFFF"/><line x1="70" y1="440" x2="572" y2="440" stroke="#1F2A44" stroke-width="2"/><line x1="70" y1="440" x2="70" y2="44" stroke="#1F2A44" stroke-width="2"/>' +
      '<text x="321" y="478" text-anchor="middle" ' + F + ' font-size="17" font-weight="700" letter-spacing="2" fill="#1F2A44">DISCIPLESHIP</text>' +
      '<text transform="translate(40,242) rotate(-90)" text-anchor="middle" ' + F + ' font-size="17" font-weight="700" letter-spacing="2" fill="#1F2A44">FORMATION</text>' +
      '<text x="88" y="76" ' + FS + '>Aging Fruit</text><text x="560" y="424" text-anchor="end" ' + FS + '>Limited Fruit</text><text x="150" y="432" ' + FS + '>Tree Planted</text><text x="560" y="62" text-anchor="end" ' + FS + '>Mature Orchard</text>' +
      ...
      /* his own X: where he said he stands, in Part Two; drawn beside our mark, never used to place it */
      (x ? '<text x="' + x.x + '" y="' + (x.y + 9) + '" text-anchor="middle" ' + F + ' font-size="28" font-weight="700" fill="#8C6A3F">×</text>' +
           '<text x="' + x.x + '" y="' + (x.y + 28) + '" text-anchor="middle" ' + F + ' font-size="13" font-weight="700" fill="#8C6A3F">your X</text>' : "") +
  ...
  /* v23 · the chapter page, set to the standard John approved Sept 25, 2026: a label over each of his lines, our line in italic above it,
     the look-back with its context, the closing form under Numbers 33, the feeling word on its own, and the map last. The story in prose lives on his story page. */
  function chapterHTML() {
    var w = W(), pt = part(), ls = lines(), m = markFor(), x = ownX(), sv = pt.stage || STAGE, sp = ROAD.stageParts({ setOut: w.setOut, camped: w.camped }), feel = null;
    ls.forEach(function (l, i) { l.i = i; if (l.k === "feel") feel = l; });
    function pick(l) { return '<button class="pickline' + (w.aloud === l.i ? " on" : "") + '" data-aloud="' + l.i + '" aria-pressed="' + (w.aloud === l.i) + '">' + esc(ROAD.endTidy(l.text)) + '</button>'; }
    var h = '<div class="sheet chapter" id="writer"><div class="ch-head"><div class="eyebrow">Walk With Me · Chapter ' + NUMS[P] + ' of ' + NUMS[ROAD_PARTS.length - 1] + '</div><div class="t">' + esc(pt.name) + '</div><div class="rule"></div></div>';
    if (pt.ours) h += '<p class="ours epi">' + esc(pt.ours) + '</p>';
    if (!ls.length && !sp.length) h += '<p class="fixed">Nothing written yet. Press Edit to go back in.</p>';
    ls.forEach(function (l) {
      if (l.k === "feel") return;
      if (l.k === "did" && l.ctx) { h += '<div class="lookback"><span class="lbl">What happened</span><span class="from">' + esc(l.ctx.lead) + '</span><p class="quote">“' + esc(ROAD.tidy(l.ctx.quote)) + '.”</p>' + pick(l) + '</div>'; return; }
      var br = l.k.charAt(0) === "q" && l.k.length <= 3 ? ROAD.bridgeFor(P, +l.k.slice(1), l.side) : "";
      h += '<div class="blk"><span class="lbl">' + esc(l.label) + '</span>' + (br ? '<p class="ours left">' + esc(br.replace(/:\s*$/, ".")) + '</p>' : "") + pick(l) + '</div>';
    });
    if (sp.length) h += '<div class="closing"><span class="lbl">Where I set out from, and where I camped</span><span class="from">' + esc(sv.ref) + ' · “' + esc(sv.text) + '”</span>' + sp.map(function (t) { return '<p class="his stageline">' + esc(t) + '</p>'; }).join("") + '</div>';
    if (feel) h += '<div class="feeling"><span class="lbl">What this stirred in me</span><span class="feelword">' + esc(feel.text.replace(/\.$/, "")) + '</span></div>';
    if (ls.length) h += '<p class="quiet">Tap one line to mark it as the one you will read aloud at group.</p>';
    if (m) {
      ...
      h += '<div class="divide"></div><div class="road"><span class="lbl">The map</span>' + markSVG(m, x) + '<p class="cap">' + leanTxt + '</p>' +
        '<p class="sub">' + (ROAD.whereTxt(m) ? esc(ROAD.whereTxt(m)) + ' ' : '') + 'This shows direction, never rank. We move from glory to glory, and the road is not straight.' + (x ? ' The X is where you said you stand: “' + esc(x.text) + '.”' : "") + '</p>' + lastPlate() + '</div>';
      if (pt.picker) {
        ...
        h += '<p class="ours left">' + lead + '</p><div class="words">' + set.map(function (s) { return '<button class="word' + (w.disc === s ? " pick" : "") + '" data-disc="' + s + '" aria-pressed="' + (w.disc === s) + '">' + s + '</button>'; }).join("") + '</div>' +
          (w.disc ? '<div class="field"><label for="t-disc">How will you practice ' + esc(w.disc) + ' this week?</label><input id="t-disc" data-w="discText" value="' + esc(w.discText) + '" placeholder="Saturday mornings at the food pantry with my son."></div>' : "");
      }
    } else h += '<div class="divide"></div><div class="road"><span class="lbl">The map</span><p class="sub mapnote">' + esc(mapNote()) + '</p>' + lastPlate() + '</div>';
  ...
  function intoStory() {
    ...
    if (pt.stand) ch.stand = { door: w.door, rel: (w.rel || []).slice(), text: (w.doorText || "").trim() };
    if (P === 1) ch.lean = leanPick();
    var m = markFor();
    if (APP.stage === "me") {
  ...
  document.addEventListener("click", function (e) {
    ...
    else if ((a = b.getAttribute("data-camp"))) { w.setOut = w.setOut === lastCamp() ? "" : lastCamp(); APP.render(true); APP.keep(); }
    else if ((a = b.getAttribute("data-door"))) { w.door = w.door === +a ? null : +a; APP.render(true); APP.keep(); }
    else if ((a = b.getAttribute("data-rel"))) { a = a.split("-"); w.rel[+a[0]] = w.rel[+a[0]] === +a[1] ? null : +a[1]; APP.render(true); APP.keep(); }
  ...
  return { trail: trail, html: function () { return W().view === "chapter" ? chapterHTML() : stepsHTML(); }, open: function (p) { P = p; var w = W(); if (w.view === "chapter") { w.view = "steps"; w.all = true; } },
...
var TELL = (function () {
  ...
  function draft(d, stage) {
    ...
    chs.forEach(function (ch, i) {
      ...
      /* v28 · his stage line (Numbers 33: where he set out from, where he camped) is his, and it travels with the chapter */
      var stg = ROAD.stageLine(ch); if (stg && stg.trim()) p.bits.push({ label: "Where I set out from, and where I camped", text: stg.trim(), ch: i, between: "" });
  ...
  /* v28 · what he has made of his story: each movement he has put together (his words, smoothed or not), where it starts, where God was.
     A movement still in lines is left to the chapters, so nothing he has not worked on is changed. */
  function crafted() {
    var out = { start: "", god: "", byMv: {}, handed: "", tailIn: false };
    if (!T || !T.parts) return out;
    out.start = (T.shape.start || "").trim(); out.god = (T.shape.god || "").trim();
    T.parts.forEach(function (p) {
      if (!p.together || !textOf(p).trim()) return;
      var t = textOf(p).trim();
      if (p.name === ROAD.HANDED) { out.handed = t; return; }
      var mv = APP.D.movements.indexOf(p.name); if (mv > 0) out.byMv[mv] = t;
      if (/Today I am camped at /.test(t)) out.tailIn = true;
    });
    return out;
  }
  ...
  function start(d, stage) {
    if (!T || T.stage !== stage) {
      T = { stage: stage, parts: null, aside: [], view: "work", showAll: false, heard: null, heardBusy: "", heardErr: "", open: "", shape: { start: "", about: "", turn: "", god: "" }, moves: [] };
  ...
  function readHTML(d) {
    ...
    var h = '<div class="sheet" id="story-read"><div class="ch-head"><div class="eyebrow">Walk With Me' + (written < all ? " · your story so far" : "") + '</div><div class="t">' + esc(d.title) + '</div><div class="by">' + esc(d.byline) + " · " + esc(d.when) + '</div><div class="rule"></div></div>' +
    ...
    if (written < all) h += '<p class="quiet center">' + written + ' of ' + all + ' chapters written. Your story grows each time you finish one.</p>' +
      (written > 1 ? '<p class="quiet center"><button class="link" data-tell="all">' + (T.showAll ? "Just the newest chapter" : "Read it all together") + '</button></p>' : "");
    var cr = crafted(), made = !!(cr.start || cr.god || Object.keys(cr.byMv).length || cr.handed);
    var blocks = chapterBlocks(d), onlyLatest = written < all && written > 1 && !T.showAll;
    if (onlyLatest) blocks = blocks.slice(-1);
    if (cr.start && !onlyLatest) h += '<p class="his prose-p">' + esc(cr.start) + '</p>';
    var shownMv = {};
    blocks.forEach(function (c) {
      ...
      /* v28 · a movement he has put together reads as his own prose, one piece */
      if (cr.byMv[c.mv]) { if (!shownMv[c.mv]) { shownMv[c.mv] = 1; cr.byMv[c.mv].split(/\n\s*\n/).forEach(function (t) { if (t.trim()) h += '<p class="his prose-p">' + esc(t.trim()) + '</p>'; }); } return; }
    ...
    if (cr.handed) { h += '<div class="movement"><span>' + esc(ROAD.HANDED) + '</span></div>'; cr.handed.split(/\n\s*\n/).forEach(function (t) { if (t.trim()) h += '<p class="his prose-p">' + esc(t.trim()) + '</p>'; }); }
    else { var hd = ROAD.handed(d, T.stage); if (hd.length) { h += '<div class="movement"><span>' + esc(ROAD.HANDED) + '</span></div>'; hd.forEach(function (l) { h += '<p class="his prose-p">' + esc(l.text) + '</p>'; }); } }
    var done = ROAD.finished(d, T.stage);
    if (done) h += (d.tail.trim() && !cr.tailIn ? '<p class="his prose-p">' + esc("Today I am camped at " + d.tail.trim()) + '</p>' : "") + (cr.god ? '<p class="his prose-p">' + esc(cr.god) + '</p>' : "") + '<p class="ours closing">' + esc(d.closes) + '</p><p class="ours lower">' + esc(ROAD.LOWER) + '</p>';
    if (done && !made) h += '<div class="invite"><span class="lbl">Make it one story</span><p>This is your story in your own lines, set in order. It is not one story yet: the story is in what happened between your lines. Press Make it one story. Answer what sits between your lines, put each movement together, and let it be read back. Then this page, and your download, carry the story you made.</p><p class="row"><button class="btn main" data-tell="work">Make it one story</button></p></div>';
    h += storyMap(d, written, done);
    ...
    h += '</div><div class="row bar">' +
      ...
      '<button class="btn' + (next && site && !course && T.stage === "me" ? "" : " main") + '" data-tell="pdf">Download</button><button class="btn" data-tell="copy">Copy</button><button class="btn" data-tell="work">' + (ROAD.finished(d, T.stage) ? "Make it one story" : "Edit") + '</button><button class="btn" data-go="page">Back</button></div>' +
  ...
  /* v25 · the road on the story page: the map with one mark for each chapter written so far; after the last chapter, the plate beneath it */
  function storyMap(d, written, done) {
    var pts = [];
    try { pts = d.mine && typeof WRITER !== "undefined" ? WRITER.trail(written) : (d.trail || []).slice(0, written); } catch (e) { pts = []; }
    pts = (pts || []).filter(Boolean);
    if (!pts.length) return "";
    if (done) ROAD.loadPlate();
    return '<div class="divide"></div><div class="road"><span class="lbl">' + (done ? "The road I walked" : "The road so far") + '</span>' + ROAD.trailSVG(pts) +
      '<p class="sub">One mark for each chapter, the newest in gold. The faint dashed line is the road on the map. This shows direction, never rank: from glory to glory.</p>' + (done ? ROAD.plateHTML() : "") + '</div>';
  }
  ...
  function storyText(d) {
    ...
    var lastMv = -1;
    ROAD.storyParts(d, T.stage, crafted()).forEach(function (p) { if (p.movement && p.mv !== lastMv) { lastMv = p.mv; out.push("— " + p.movement.toUpperCase() + " —", ""); } if (p.label) out.push(p.label.toUpperCase()); if (p.ours) out.push(p.ours); out.push(p.text, ""); });
    out.push("Written in Walk With Me · Ancient Path Biblical Coaching");
    return out.join("\n");
  }
  ...
  function click(b) {
    ...
    if (act === "gaps") askGaps(i);
    ...
    else if (act === "all") { T.showAll = !T.showAll; APP.render(); }
      ...
      Promise.all([Promise.resolve(use), ready]).then(function (r) { var dl = r[0];
        if (window.AP_ROAD && window.jspdf) { APP.saveFile(ROAD.pdf(window.jspdf.jsPDF, D, T.stage, crafted()).output("blob"), "the-road-i-walked.pdf"); APP.say("Downloaded."); return; }
        if (!dl || !window.jspdf) { APP.say("Download is not available in this view."); return; }
        var blob = ROAD.pdf(window.jspdf.jsPDF, D, T.stage, crafted()).output("blob");
...
(function () {
  ...
  /* v24 · the site knows who is signed in; his name is the byline from the first save, never "Your name" */
  function siteName() {
    try { var m = window.me || (window.parent && window.parent.me); if (!m) return "";
      var f = m.firstname || m.firstName || m.first_name || "", l = m.lastname || m.lastName || m.last_name || "", u = m.username || m.name || "";
      var full = (f + " " + l).trim(); return full || (typeof u === "string" && u.indexOf("@") < 0 ? u : ""); } catch (e) { return ""; }
  }
  function emptyMan() {
    var m = JSON.parse(JSON.stringify(ROAD_DATA));
    m.byline = siteName() || "Your name"; m.tail = ""; m.bends = []; m.trail = []; m.marks = []; m.meCount = 0; m.mine = true;
  ...
  function map() {
    var chs = ROAD.chapters(D, stage), pts = D.mine ? WRITER.trail(chs.length) : D.trail.slice(0, chs.length), last, d;
    if (pts.length < 2) return '<p class="quiet center">' + (pts.length ? "Your trail appears on the map once two of your chapters have a mark." : "Your mark appears once both sides of the map have an answer. That begins in Part Two, when you say where you stand.") + '</p>';
  ...
  function storyScreen() {
    ...
    h += '<div class="road">' + map() + '<p class="cap">' + (walking ? "This is the road you have walked so far." : "This is the road you walked.") +
      ...
      '<p class="sub">The faint dashed line is the road on the map. The gold trail is yours, one mark for each section. It shows direction, never rank: from glory to glory.</p>' + (walking ? "" : ROAD.plateHTML()) + '</div>' +
  ...
  function restore(o) { if (!o || !o.me) return; ME = o.me; ME.mine = true; if (!ME.byline || ME.byline === "Your name") ME.byline = siteName() || "Your name"; WRITER.load(o.w); meT = o.t || null; if (stage === "me") { D = ME; TELL.load(meT); render(true); } }
  function openStore() {
  ...
  function download() {
    ...
    /* v27 · the plate is fetched before the PDF is made, so a finished story always ends on it */
    var ready = new Promise(function (res) { ROAD.loadPlate(function () { res(); }); });
    if (SITE) { if (!window.jspdf) { say("The PDF could not be made just now. Print still works."); return; } ready.then(function () { saveFile(ROAD.pdf(window.jspdf.jsPDF, D, stage, TELL.crafted(D, stage)).output("blob"), "the-road-i-walked.pdf"); say("Downloaded."); }); return; }
    var use = window.claude && window.claude.use ? window.claude.use("downloads") : Promise.resolve(null);
    Promise.all([Promise.resolve(use), ready]).then(function (r) { var dl = r[0];
      if (!dl || !window.jspdf) { say("Download is not available in this view. The same PDF is attached in our chat."); return; }
      var blob = ROAD.pdf(window.jspdf.jsPDF, D, stage, TELL.crafted(D, stage)).output("blob");
