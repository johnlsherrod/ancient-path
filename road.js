/* AP-ROAD-v1 (v23: no mark until both sides of the map are answered; Where I stand in Part Two; the lean statement with three answers and his own X; the chapter page set to the approved standard; clean seams) · The Road I Walked · Walk With Me. One file: styles, the ten chapters, the story, the save. Built from the walk-through modules. */
(function(){
...
var WRITER = (function () {
  ...
  function markFor() {
    var w = W(), f = [], dd = [];
    Q().forEach(function (q, i) { var v = w.n[i]; if (v == null || !q.side) return; v = q.back ? 6 - v : v; (q.side === "F" ? f : dd).push(v); });
    var F = ROAD.avg(f), Dn = ROAD.avg(dd);
    if (Dn == null) Dn = part().stand ? standD(w) : (P > 1 ? standD(baseline()) : null);
    return ROAD.markFD(F, Dn);
  }
  function leanPick() {
  ...
  function ownX() {
    var k = null;
    if (P === 1) k = leanPick();
    else if (P > 1) { var ch = APP.D.chapters[1]; k = ch && ch.picks ? ch.picks[0] : null; }
    if (k == null) return null;
    var pt = k === 0 ? ROAD.markFD(4, 2) : k === 1 ? ROAD.markFD(2, 4) : k === 2 ? ROAD.markFD(3.5, 3.5) : ROAD.markFD(1.5, 1.5), t = leanOps()[k] || "";
  ...
  function steps() { var s = ["intro"]; if (lastWeek()) s.push("follow"); if (part().evidence) s.push("witness"); Q().forEach(function (q, i) { s.push("q" + i); if (i === 0 && part().stand) s.push("stand"); }); if (P > 0) s.push("between"); s.push("feel", "stage"); return s; }
